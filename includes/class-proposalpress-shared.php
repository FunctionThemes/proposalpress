 ...
 * Calls the class.
 */
function proposalpress_call_shared_class() {
    new ProposalPress_Shared();

}
add_action( 'proposalpress_loaded', 'proposalpress_call_shared_class', 2 );



class ProposalPress_Shared {
	...
	private static $options = array(
		'general' =>  'proposalpress_general',
		'business' =>  'proposalpress_business',
		'quotes' =>  'proposalpress_quotes',
		'invoices' =>  'proposalpress_invoices',
		'payments' =>  'proposalpress_payments',
		'translate' =>  'proposalpress_translate'
	);


	public static function get_proposalpress_option( $get_option ) {
		$proposalpress_option = get_option( self::$options[$get_option] );
		return $proposalpress_option;
	}

	public static function get_proposalpress_options() {
		foreach ( self::$options as $option_name => $option ) {
			$proposalpress_options[$option_name] = get_option( $option );
		}
		return $proposalpress_options;
	}
	...
	public static function get_item_id( $id = 0 ) {
		...
		$payments = get_option( 'proposalpress_payments' );
		...
		if ( isset( $id ) && ! empty( $id ) ) {
			...
			return (int) $_POST['id'];
		} else if ( isset( $_POST['proposalpress_payment_invoice_id'] ) && ! empty( $_POST['proposalpress_payment_invoice_id'] ) && $_POST['proposalpress_payment_invoice_id'] != $payment_page ) {
			return (int) $_POST['proposalpress_payment_invoice_id'];
		}
	...
	public static function get_type( $id = 0 ) {
		...
		if ( $type == 'proposalpress_quote' || $type == 'proposalpress_invoice' ) {
			return str_replace( 'proposalpress_', '', $type );
		}
	...
	public static function get_filename( $id = 0 ) {
		...
		$type = self::get_type();
		$filename = sanitize_file_name( strtolower( $type . '-' . proposalpress_get_prefix( $id ) . proposalpress_get_number( $id ) ) );
		return apply_filters( 'proposalpress_get_the_filename', $filename, $id );
	}
	...
	public static function get_tax_name() {
		$payments 	= self::get_proposalpress_option( 'payments' );
	    $name 		= $payments['tax_name'] ? $payments['tax_name'] : __( 'Tax', 'proposalpress-invoices' );
    	return $name;
	}
	...
	public static function get_tax_amount( $id = 0 ) {
		...
		$id = ProposalPress_Shared::get_item_id( $id );

	    if ( isset( $id ) ) {

	    	$amount = self::get_proposalpress_meta( $id, '_proposalpress_tax', false );
	...
	public static function get_currency_symbol() {
	    ...
	    if ( isset( $id ) ) {
	    	...
	    	$symbol = self::get_proposalpress_meta( $id, '_proposalpress_currency_symbol', false );
	    	if( !empty( $symbol ) ) {
	    		$symbol 	= $symbol[0];
	    	} else {
	    		$payments 	= self::get_proposalpress_option( 'payments' );
	...
	public static function get_raw_number( $amount ) {
		$thou_sep 	= proposalpress_get_thousand_seperator();
	    $dec_sep 	= proposalpress_get_decimal_seperator();
	    ...
	    return apply_filters( 'proposalpress_get_raw_number', $amount );
	}
	...
	public static function get_formatted_number( $amount ) {
	    ...
	    return apply_filters( 'proposalpress_get_formatted_number', $formatted );
	}
	...
	public static function get_formatted_currency( $amount ) {
	    ...
	    $symbol 	= proposalpress_get_currency_symbol();
	    $position 	= proposalpress_get_currency_position();
	    ...
	    return apply_filters( 'proposalpress_get_formatted_currency', $formatted );
	}
	...
	public static function get_line_items( $id = 0 ) {
		$items = self::get_proposalpress_meta( $id, '_proposalpress_items', false );
		return $items;
	}
	...
	public static function get_totals( $id ) {
	    ...
	    $sub_total = array_sum( $total );
	   	...
	   	$global_tax = apply_filters( 'proposalpress_totals_global_tax', self::get_tax_amount( $id ), $id );
	    ...
	    return apply_filters( 'proposalpress_invoice_totals', array(
	        'sub_total' 		=> $sub_total,
	        'tax' 				=> $tax_amount,
	        'total' 			=> $total,
	    ), $id );
	...
	public static function get_line_item_sub_total( $qty, $amt, $tax ) {
        ...
        // pp($amt);
	    return apply_filters( 'proposalpress_get_line_item_sub_total', $line_total );
	}
    ...
    public static function get_accepted_payment_methods() {
        ...
        return apply_filters( 'proposalpress_register_payment_method', $pay_array );
    }
	...
	public static function get_business_details() {
		...
		$options = self::get_proposalpress_options();

	    return apply_filters( 'proposalpress_business_details', array(
	...
	public static function get_client_id( $id = 0 ) {
		...
		$client_id = self::get_proposalpress_meta( $id, '_proposalpress_client' );
		return $client_id;
	}
	...
	public static function get_client_details( $id = 0 ) {
		...
		$client = apply_filters( 'proposalpress_client_data', self::get_client_data( $id ) );
		$id     = apply_filters( 'proposalpress_client_id', self::get_client_id( $id ) );

		if ( ! $client ) {
			return;
		}

		return apply_filters( 'proposalpress_client_details', array(
			'id'         => $client->ID,
			'first_name' => isset( $client->first_name ) ? $client->first_name : '',
			'last_name'  => isset( $client->last_name ) ? $client->last_name : '',
			'business'   => get_user_meta( $client->ID, '_proposalpress_client_business', true ),
			'address'    => get_user_meta( $client->ID, '_proposalpress_client_address', true ),
			'extra_info' => get_user_meta( $client->ID, '_proposalpress_client_extra_info', true ),
